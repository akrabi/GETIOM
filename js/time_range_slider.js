        ...
        create: function( event, ui ) {
            ...
            (wait = function() {
                if ((container = target.parents('.content')).length != 0) {
                    handles.eq(0).tooltip({animation: false, placement: 'bottom',trigger: 'manual', container: container, title: getTime(startVal)}).tooltip('show');
                    handles.eq(1).tooltip({animation: false, placement: 'bottom',trigger: 'manual', container: container, title: getTime(endVal)}).tooltip('show');
                } else {
                    setTimeout( wait, 50 );
                }
            })();
